...
class SharePoint:
    def __init__(
        ...
        # Làm sạch download dir
        self.__clean_download_dir()

    @HandleException
    def __clean_download_dir(self):
        for file in os.listdir(self.download_directory):
            filePath = os.path.join(self.download_directory, file)
            try:
    ...
    def get_files_in_folder(
        self, site_folder: str, folder: str
    ) -> tuple[str, list[str]]:
    ...
    def download_file(self, site_url: str, file_pattern: str) -> tuple[bool, list]:
            ...
            for row in rows:
                ...
                if icon_gridcell.find_elements(By.TAG_NAME, "svg"):  # Folder
                    ...
                    if pattern.match(display_name):
                        gridcell_div = row.find_element(
                            By.XPATH, "./preceding-sibling::div[@role='gridcell']"
                        )
                        checkbox = gridcell_div.find_element(
                            By.CSS_SELECTOR, "div[role='checkbox']"
                        )
                        if checkbox.get_attribute("aria-checked") == "false":
                            time.sleep(1)
                            download_file.append(
                                os.path.join(self.download_directory, display_name)
                            )
                            checkbox.click()
                            found_file = True
        except TimeoutException:
                ...
                if icon_gridcell.find_elements(By.TAG_NAME, "svg"):  # Folder
                    ...
                    if pattern.match(span_name_gridcell.text):
                        if row.find_elements(
                            ...
                            download_file.append(
                                os.path.join(
                                    self.download_directory, span_name_gridcell.text
                                )
                            )
                            row.find_element(
                                By.CSS_SELECTOR, "div[class^='rowSelectionCell_']"
        ...
        try:
            ...
            self.wait.until(
                EC.presence_of_element_located(
                    (By.CSS_SELECTOR, "i[data-icon-name='More']")
                )
            )
            self.wait.until(
        ...
        time.sleep(2)
        self.logger.info(f"Tải {file_pattern} - {site_url} thành công")
